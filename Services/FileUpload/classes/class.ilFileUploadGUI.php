...
class ilFileUploadGUI
{
	...
	protected function getSharedHtml()
	{
        ...
        if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
            if (strpos($this->drop_zone_id, "ilFileUploadDropZone_") !== false) {
                ...
                $copyRightRadio = copyrightHelper::_getCopyRightActiveOptionRadio("", "copy_right_option_{%=o.id%}")->render();
                $copyRightRadioForZip = copyrightHelper::_getCopyRightActiveOptionRadio("", "copy_right_option_zip_file_{%=o.id%}")->render();

                $copyRightRadio = str_replace("<script", "<!-- <script", $copyRightRadio);
                $copyRightRadio = str_replace("</script>", "</script> -->", $copyRightRadio);
                $copyRightRadioForZip = str_replace("<script", "<!-- <script", $copyRightRadioForZip);
                $copyRightRadioForZip = str_replace("</script>", "</script> -->", $copyRightRadioForZip);

                $tpl_row->setVariable("TEXT_COPY_RIGHT", $copyRightPlugin->txt('select_list_title'));
                $tpl_row->setVariable("COPY_RIGHT_PLUGIN_IS_ACTIVE", ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')));
                $tpl_row->setVariable("COPY_RIGHT_OPTIONS", $copyRightRadio);
                $tpl_row->setVariable("COPY_RIGHT_OPTIONS_ZIP_FILE", $copyRightRadioForZip);
            } else {
                $tpl_row->setVariable("COPY_RIGHT_PLUGIN_IS_ACTIVE", false);
            }
        }
