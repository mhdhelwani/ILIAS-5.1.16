	...
	function listFiles()
	{
		...
		if ($this->getAllowFileCreation())
		{
            ...
            if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                ...
                if (strpos($this->getTableId(), "excassfil") === 0 ||
					strpos($this->getTableId(), "excfbfil") === 0 ||
					strpos($this->getTableId(), "htlmfs") === 0 ||
					strpos($this->getTableId(), "mobfs") === 0) {
                    $ilToolbar->addInputItem(copyrightHelper::_getCopyRightActiveOptionSelectList("select_list_title", "copy_right_option"), true);
                }
            }
	...
	function renameFile()
	{
        ...
        if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
            ...
            if (strpos($this->getTableId(), "excassfil") === 0) {
                ...
                $exercise = new ilObjExercise($_GET["ref_id"]);
                $objId = $exercise->getId();
                $objFound = true;
                $fileLocation = "exc_instruction_files|%s|";
            } else if (strpos($this->getTableId(), "excfbfil") === 0 && $_GET["member_id"]) {
                require_once './Modules/Exercise/classes/class.ilObjExercise.php';

                $exercise = new ilObjExercise($_GET["ref_id"]);
                $objId = $exercise->getId();
                $objFound = true;
                $fileLocation = "ass_feedback|%s|" . $_GET["member_id"];
            } else if (strpos($this->getTableId(), "htlmfs") === 0) {
                require_once './Modules/HTMLLearningModule/classes/class.ilObjFileBasedLM.php';

                $lmsObj = new ilObjFileBasedLM($_GET["ref_id"]);
                $objId = $lmsObj->getId();
                $objFound = true;
                $fileLocation = "lms_html_file|%s|";
            } else if (strpos($this->getTableId(), "mobfs") === 0) {
                require_once './Services/MediaObjects/classes/class.ilObjMediaObject.php';

                $mobId = substr($this->getTableId(), 5);
                $mobObj = new ilObjMediaObject($mobId);
                $objId = $mobObj->getId();
                $objFound = true;
                $fileLocation = "mob|%s|";
            }
            ...
            if ($objFound) {
                if (@is_dir($dir.$new_name)){
                    $dir_files = iterator_to_array(new RecursiveIteratorIterator(new RecursiveDirectoryIterator($dir.$new_name)));
                    ...
                    foreach ($dir_files as $f => $d) {
                        ...
                        if (!is_dir($f)) {
                            $sub_dir = str_replace(
                                "\\" ,
                                "/",
                                substr($pi["dirname"], strlen($dir) + strlen($new_name))
                            );

                            $new_sub_dir =  $new_name . $sub_dir;
                            $old_sub_dir = $_GET["old_name"]. $sub_dir;
                            ...
                            copyrightHelper::_updateCopyRightOptionByObjectId(
                                ...
                                copyrightHelper::_getCopyRightValue(
                                    $objId,
                                    0,
                                    sprintf($fileLocation, $pi["basename"]) . "|" .
									((!empty($cur_subdir)) ?  "/" . $cur_subdir . "/" . $old_sub_dir : $old_sub_dir)
                                ),
                                0,
                                sprintf($fileLocation, $pi["basename"]) . "|" .
								((!empty($cur_subdir)) ?   "/" . $cur_subdir . "/" . $new_sub_dir :  $new_sub_dir)
							);
                    ...
                    copyrightHelper::_updateCopyRightOptionByObjectId(
                        ...
                        copyrightHelper::_getCopyRightValue(
                            $objId,
                            0,
                            sprintf($fileLocation, $_GET["old_name"]) . "|" .
                             ((!empty($cur_subdir)) ? "/" . $cur_subdir : " ")
                        ),
                        0,
                        sprintf($fileLocation, $new_name) . "|" . ((!empty($cur_subdir)) ? "/" . $cur_subdir : " ")
					);
			...
			if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
				...
				if ($objFound){
                    copyrightHelper::_updateCopyRightOptionByObjectId(
                        ...
                        0,
                        sprintf($fileLocation, ilUtil::stripSlashes($_FILES["new_file"]["name"])) . "|" .
                        ((!empty($cur_subdir)) ? "/" . $cur_subdir : " "));
				}
            }
                ...
                if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                    ...
                    if ($objFound) {
                        copyrightHelper::_updateCopyRightOptionByObjectId(
                            ...
                            $_POST["copy_right_option"],
                            0,
                            sprintf($fileLocation, ilUtil::stripSlashes($_POST["uploaded_file"])) . "|" .
                            ((!empty($cur_subdir)) ? "/" . $cur_subdir : " "));
					}
                }
                ...
                if ($objFound) {
                    if (@is_file($file)) {
                        copyrightHelper::_deleteCopyRightOptionByObjectId(
                            $objId,
                            0,
                            sprintf($fileLocation, ilUtil::stripSlashes($pi["basename"])) . "|" .
                            ((!empty($cur_subdir)) ? "/" . $cur_subdir : " "));
                    }

                    if (@is_dir($file)) {
                        $dir_files = iterator_to_array(new RecursiveIteratorIterator(new RecursiveDirectoryIterator($file)));

                        foreach ($dir_files as $f => $d) {
                            $pi1 = pathinfo($f);

                            if (!is_dir($f) && $pi["basename"] !== "..") {
                                $sub_dir = str_replace(
                                    "\\",
                                    "/",
                                    substr($pi1["dirname"], strlen($this->main_dir))
                                );

                                copyrightHelper::_deleteCopyRightOptionByObjectId(
                                    $objId,
                                    0,
                                    sprintf($fileLocation, ilUtil::stripSlashes($pi1["basename"])) .
                                    "|" . ((!empty($sub_dir)) ? $sub_dir : " "));
                            }
                        }
                    }
				}
            ...
            if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                ...
                if ($objFound) {
                    foreach ($diff_r as $f => $d) {
                        ...
                        if (!is_dir($f) && $this->isValidSuffix(strtolower($pi["extension"]))) {
                            $sub_dir = str_replace("\\" , "/", substr($pi["dirname"], strlen($cur_dir)));
                            copyrightHelper::_updateCopyRightOptionByObjectId(
                                $objId,
                                copyrightHelper::_getCopyRightValue(
                                    $objId,
                                    0,
                                    sprintf($fileLocation, basename($a_file)) . "|" .
                                    ((!empty($cur_subdir)) ? "/" . $cur_subdir : " ")
                                ),
                                0,
                                sprintf($fileLocation, $pi["basename"]) . "|" .
								((!empty($cur_subdir)) ? "/" . $cur_subdir . $sub_dir : (empty($sub_dir) ? " " : $sub_dir)));
                        }
                    }
				}
            }
