	...
	*/
	...
	protected function initImportForm($a_new_type)
	{
        ...
        if ( $GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
            ...
            $notIncludeType = ["orgu", "sess"];
            if (!in_array($a_new_type, $notIncludeType)) {
                $form->addItem(copyrightHelper::_getCopyRightActiveOptionRadio("select_list_title", "copy_right_option_imp"));
            }
        }
		...
		if ($form->checkInput())
		{
			...
			try
			{
				$new_id = $imp->importObject(null, $_FILES["importfile"]["tmp_name"],
					$_FILES["importfile"]["name"], $new_type /* patch start 'mhd_helwani' */, "", false, $_REQUEST["copy_right_option_imp"] /* patch end 'mhd_helwani' */);
