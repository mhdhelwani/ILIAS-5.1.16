...
class ilObjMediaObjectGUI extends ilObjectGUI
{
	...
	function initForm($a_mode = "create")
	{
		...
		$op1 = new ilRadioOption($lng->txt("cont_file"), "File");
			...
			if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                ...
                $op1->addSubItem(copyrightHelper::_getCopyRightActiveOptionRadio("select_list_title", "copy_right_option_Standard"));
			}
        ...
        if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
            ...
            $op2->addSubItem(copyrightHelper::_getCopyRightActiveOptionRadio("select_list_title", "copy_right_option_Fullscreen"));
        }
	...
	*/
	public function getValues()
	{
        ...
        if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
            ...
            $mediaItems = $this->object->getMediaItems();

            foreach ($mediaItems as $mediaItem) {
                if ($mediaItem->getLocationType() === "LocalFile") {
                    $values['copy_right_option_'. $mediaItem->getPurpose()] = copyrightHelper::_getCopyRightValue(
                        $this->object->getId(),
                        $mediaItem->getId(),
	...
	*/
	static function setObjectPerCreationForm($a_mob)
	{
		...
		$mediaItems = $a_mob->getMediaItems();
			...
			if ($mediaItem->getLocationType() !== "LocalFile") {
				...
				if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
					copyrightHelper::_updateCopyRightOptionByObjectId(
						$a_mob->getId(),
						$_POST['copy_right_option_'. $mediaItem->getPurpose()] ,
						$mediaItem->getId(),
						"mob_" . $mediaItem->getPurpose());
				}
			}
	...
	function savePropertiesObject()
	{
		...
		if ($this->form_gui->checkInput())
		{
            ...
            if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                ...
                if ($this->object->hasFullScreenItem()) {
                    $copy_right_value_full_screen = copyrightHelper::_getCopyRightValue(
                        $this->object->getId(),
                        $this->object->getMediaItem("Fullscreen")->getId(),
                        "mob_Fullscreen"
                    );
                    $full_screen_old_file_name = $this->object->getMediaItem("Fullscreen")->getLocation();
                    ...
                    $copy_right_value_full_screen = null;
                    $full_screen_old_file_name = "";
                }

                $copy_right_value_standard = copyrightHelper::_getCopyRightValue(
                    $this->object->getId(),
                    $this->object->getMediaItem("Standard")->getId(),
                    "mob_Standard"
                );
                $standard_old_file_name = $this->object->getMediaItem("Standard")->getLocation();
...
//echo "-".$_POST["full_size"]."-".is_int(strpos($format, "image"))."-".$full_item->getLocationType()."-";
			...
			foreach ($mediaItems as $mediaItem) {
                if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                    if ($mediaItem->getPurpose() == "Standard") {
                        if ($mediaItem->getLocation() != $standard_old_file_name) {
                            $dir = substr($standard_old_file_name, 0, strripos($standard_old_file_name, "/"));
                            $file_name = substr($standard_old_file_name, strlen($dir));

                            copyrightHelper::_updateCopyRightOptionByObjectId(
                                $this->object->getId(),
                                $copy_right_value_standard,
                                0,
                                "mob|" . $file_name . "|" . (empty($dir) ? " " : "/" . $dir)
                            );
                        }
                    } else if ($mediaItem->getPurpose() == "Fullscreen") {
                        if ($mediaItem->getLocation() != $full_screen_old_file_name) {
                            $dir = substr($full_screen_old_file_name, 0, strripos($full_screen_old_file_name, "/"));
                            $file_name = substr($full_screen_old_file_name, strlen($dir));

                            copyrightHelper::_updateCopyRightOptionByObjectId(
                                $this->object->getId(),
                                $copy_right_value_full_screen,
                                0,
                                "mob|" . $file_name . "|" . (empty($dir) ? " " : "/" . $dir)
                            );
                        }
	...
	{						
        ...
        if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
            ...
            if ($this->object->hasFullScreenItem()) {
                ...
                $full_screen_file_name = $this->object->getMediaItem("Fullscreen")->getLocation();
            ...
            $standard_file_name = $this->object->getMediaItem("Standard")->getLocation();
        ...
        if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
            $media_items = $this->object->getMediaItems();

            foreach ($media_items as $media_item) {
                if ($media_item->getPurpose() == "Fullscreen" && $copy_right_value_full_screen) {
                    copyrightHelper::_updateCopyRightOptionByObjectId(
                        $this->object->getId(),
                        $copy_right_value_full_screen,
                        $media_item->getId(),
                ...
                if ($media_item->getPurpose() == "Standard") {
                    if ($copy_right_value_standard && $full_screen_file_name != $standard_file_name) {
                        $dir = substr($standard_file_name, 0, strripos($standard_file_name, "/"));
                        $file_name = substr($standard_file_name, strlen($dir));

                        copyrightHelper::_updateCopyRightOptionByObjectId(
                            $this->object->getId(),
                            $copy_right_value_standard,
                            0,
                            "mob|" . trim($file_name, "/") . "|". (empty($dir) ? " " : "/" . $dir)
                        );
                    }

                    if ($media_item->getLocation() == $full_screen_file_name){
                        $copy_right_value_standard = $copy_right_value_full_screen;
                    } else {
                        $dir = substr($media_item->getLocation(), 0, strripos($media_item->getLocation(), "/"));
                        $file_name = substr($media_item->getLocation(), strlen($dir));
	...
	*/
	...
	{		
        ...
        if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
            ...
            foreach ($media_items as $media_item) {
                if ($media_item->getPurpose() == "Standard" && $copy_right_value_standard) {
                    copyrightHelper::_updateCopyRightOptionByObjectId(
                        $this->object->getId(),
                        $copy_right_value_standard,
                        $media_item->getId(),
                ...
                if ($media_item->getPurpose() == "Fullscreen") {
                    if ($copy_right_value_full_screen && $full_screen_file_name != $standard_file_name) {
                    	$dir = substr($full_screen_file_name, 0, strripos($full_screen_file_name, "/"));
                    	$file_name = substr($full_screen_file_name, strlen($dir));
                    ...
                    if ($media_item->getLocation() == $standard_file_name){
                        $copy_right_value_full_screen = $copy_right_value_standard;
                    } else {
                        $dir = substr($media_item->getLocation(), 0, strripos($media_item->getLocation(), "/"));
                        $file_name = substr($media_item->getLocation(), strlen($dir));
