...
//var_dump($a_rec);
				...
				if ($dir != "" && $this->getImportDirectory() != "")
				{
                    ...
                    if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                        ...
                        $overlayImages = $newObj->getFilesOfDirectory("overlays");
                        foreach ($overlayImages as $overlayImage) {
                            copyrightHelper::_updateCopyRightOptionByObjectId(
                                $newObj->getId(),
                                $this->getCopyRightOption(),
                                0,
                                "interactive_overlay_image|" . $overlayImage);
                        }

                        $dir_files = iterator_to_array(new RecursiveIteratorIterator(new RecursiveDirectoryIterator($newObj->getDataDirectory())));
                        foreach ($dir_files as $f => $d) {
                            ...
                            if (!is_dir($f)) {
                                $sub_dir = str_replace(
                                    "\\",
                                    "/",
                                    substr($pi["dirname"], strlen($newObj->getDataDirectory()))
                                );
...
//echo "<br>++add++"."0:".$a_rec["Id"].":mob+0:".$newObj->getId().":mob"."+";
			...
			case "mob_media_item":
                ...
                if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                    ...
                    if ($newObj->getLocationType() === "LocalFile") {
                        copyrightHelper::_updateCopyRightOptionByObjectId(
                            $mob_id,
                            $this->getCopyRightOption(),
                            $newObj->getId(),
                            "mob_" . $newObj->getPurpose());

                        if ($newObj->getLocation() == basename($newObj->getLocation())){
                            $dir = " ";
                        } else {
                            $dir = "/" . substr(
                                    $newObj->getLocation(),
                                    0,
                                    (strlen($newObj->getLocation()) - strlen(basename($newObj->getLocation())) - 1));
                        }
                        if (copyrightHelper::_getCopyRightValue(
                            $mob_id,
                            0,
                            "mob|" . basename($newObj->getLocation()) . "|" . $dir)){
