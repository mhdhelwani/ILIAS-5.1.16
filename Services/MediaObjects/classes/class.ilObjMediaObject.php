	...
	function create($a_create_meta_data = false, $a_save_media_items = true)
	{
		...
		if ($a_save_media_items)
		{
			...
			for($i=0; $i<count($media_items); $i++)
			{
				...
				$oldItemId = $media_items[$i]->getId();
				$oldMobId = $media_items[$i]->getmobId();
...
//var_dump($linked);
	...
	function duplicate()
	{
        ...
        if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
            require_once('./Customizing/global/plugins/Services/UIComponent/UserInterfaceHook/CopyRight/classes/Helper/class.copyrightHelper.php');
            foreach ($new_obj->getFilesOfDirectory("overlays") as $overlaysFile) {
                copyrightHelper::_updateCopyRightOptionByObjectId(
                    $new_obj->getId(),
                    copyrightHelper::_getCopyRightValue(
                        $this->getId(),
                        0,
                        "interactive_overlay_image|" . $overlaysFile
                    ),
                    0,
                    "interactive_overlay_image|" . $overlaysFile);
            }

            $dir_files = iterator_to_array(new RecursiveIteratorIterator(new RecursiveDirectoryIterator($new_obj->getDataDirectory())));
            foreach ($dir_files as $f => $d) {
                ...
                if (!is_dir($f)) {
                    $sub_dir = str_replace(
                        "\\",
                        "/",
                        substr($pi["dirname"], strlen($new_obj->getDataDirectory()))
                    );
