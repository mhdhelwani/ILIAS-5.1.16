...
class ilForumXMLParser extends ilSaxParser
{
    ...
    var $copy_right_option;

    /**
     * get copy_right_option
     *
     * @access	public
     * @return	string	 copy_right_option
     */
    function getCopyRightOption()
    {
        return $this->copy_right_option;
    }

    /**
     * set copy_right_option
     *
     * @access	public
     * @param	int
     */
    function setCopyRightOption($a_copy_right_option)
    {
        $this->copy_right_option = $a_copy_right_option;
    }
				...
				case 'Attachment':
					...
					if( strlen($importPath) )
					{
                        ...
                        if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                            ...
                            $file_name = substr(
                                $newFilename,
                                strlen($this->forum->getId() . '_' . $this->lastHandledPostId . '_')
                            );

                            copyrightHelper::_updateCopyRightOptionByObjectId(
                                $this->forum->getId(),
                                $this->getCopyRightOption(),
                                $this->lastHandledPostId,
                                "forum|" . $file_name
                            );
                        }
