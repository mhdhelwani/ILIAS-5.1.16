...
{	
	 ...
	 */
	function uploadFile($a_http_post_files, $unzip = false/* patch start 'mhd_helwani' */ ,$copy_right_option = null/* patch end 'mhd_helwani' */)
	{
		global $ilDB;
		...
		{			
            ...
            if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                ...
                copyrightHelper::_updateCopyRightOptionByObjectId(
                    ...
                    "exc_return|" . $a_http_post_files["name"] . "|" . $next_id
                );
            }
	...
	*/
	function processUploadedZipFile($fileTmp/* patch start 'mhd_helwani' */ ,$copy_right_option = null /* patch end 'mhd_helwani' */)
	{		
		global $lng; 
		...
		try 
		{
			...
			if($success)
			{
				foreach ($filearray["file"] as $key => $filename)
				{
					...
					if(!$this->uploadFile($a_http_post_files, true/* patch start 'mhd_helwani' */ ,$copy_right_option /* patch end 'mhd_helwani' */))
					{
						$success = false;
						ilUtil::sendFailure($lng->txt("exc_upload_error"), true);
					}							
				...
				foreach ($result_array as $key => $value)
				{
					if($value["filename"])
					{
                        ...
                        copyrightHelper::_deleteCopyRightOptionByObjectId(
                            $value["obj_id"],
                            $value["ass_id"],
                            "exc_return|" . basename($value["filetitle"] ."|" .$value["returned_id"])
                        );
