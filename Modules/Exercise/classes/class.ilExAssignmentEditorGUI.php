...
class ilExAssignmentEditorGUI 
{
	...
	*/
	protected function initAssignmentForm($a_type, $a_mode = "create")
	{
		...
		if ($a_mode == "create")
		{
            ...
            if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                ...
                $copyRightOptionRadio = copyrightHelper::_getCopyRightActiveOptionRadio("select_list_title", "copy_right_option_instruction_files");
                $copyRightOptionRadio->setRequired(false);
                $files->addSubItem($copyRightOptionRadio);
            }
		...
		$form->addItem($fb);
            ...
            if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                ...
                $fb_file->addSubItem(copyrightHelper::_getCopyRightActiveOptionRadio("select_list_title", "copy_right_option"));
            }
	...
	protected function processForm(ilPropertyFormGUI $a_form)
	{
		...
		{									
			...
			if($valid)
			{
                ...
                if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                    $res["copy_right_option"] = $a_form->getInput("copy_right_option");
                    $res["copy_right_option_instruction_files"] = $a_form->getInput("copy_right_option_instruction_files");
                }
	...
	{			
        ...
        if ($a_ass->getFeedbackFile()) {
            if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                copyrightHelper::_updateCopyRightOptionByObjectId(
                    $a_ass->getExerciseId(),
                    $a_input["copy_right_option"],
                    $a_ass->getId(),
        ...
        if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
            if (is_array($a_input["files"])) {
                if (is_array($a_input["files"]["name"])) {
                    foreach ($a_input["files"]["name"] as $k => $name) {
                        if ($name != "") {
                            copyrightHelper::_updateCopyRightOptionByObjectId(
                                $a_ass->getExerciseId(),
                                $a_input["copy_right_option_instruction_files"],
                                $a_ass->getId(),
                                "exc_instruction_files|" . $name);
                        }
                    }
                }
            }
	...
	*/
	...
	public function getAssignmentValues(ilPropertyFormGUI $a_form)
	{
        ...
        if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
            ...
            $a_form->getItemByPostVar("copy_right_option")->setValue(copyrightHelper::_getCopyRightValue(
