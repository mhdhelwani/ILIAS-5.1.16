	...
	*/
	...
	protected function initUploadForm()
	{
        ...
        if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
            ...
            $copyRightOptionRadio = copyrightHelper::_getCopyRightActiveOptionRadio("select_list_title", "copy_right_option");
            $form->addItem($copyRightOptionRadio);
        }
	...
	function uploadFileObject()
	{
		...
		else
		{
			...
			foreach ($_FILES["deliver"]["name"] as $k => $v)
			{
				...
				if(!$this->submission->uploadFile($file /* patch start 'mhd_helwani' */ ,false, $_POST["copy_right_option"] /* patch end 'mhd_helwani' */))
				{
	...
	function uploadZipObject()
	{
		...
		else 
		{
			...
			{
				if($this->submission->processUploadedZipFile($_FILES["deliver"]["tmp_name"] /* patch start 'mhd_helwani' */ ,$_POST["copy_right_option"] /* patch end 'mhd_helwani' */))
				{
					ilUtil::sendSuccess($this->lng->txt("file_added"), true);				
					$this->handleNewUpload();				
				}
