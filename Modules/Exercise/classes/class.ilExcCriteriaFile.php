...
class ilExcCriteriaFile extends ilExcCriteria
{
	...
	{		
        ...
        foreach ($this->getFiles() as $file) {
            copyrightHelper::_deleteCopyRightOptionByObjectId(
                ...
                $this->ass->getId(),
                "peer_feedback|".basename($file)."|".$this->getId()."|".$this->peer_id);
        }
	...
	public function addToPeerReviewForm($a_value = null)
    {
        ...
        if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
            ...
            $copyRightOptionRadio = copyrightHelper::_getCopyRightActiveOptionRadio("select_list_title", "copy_right_option_" . $this->getId());
            $copyRightOptionRadio->setRequired($this->isRequired());
	...
	public function importFromPeerReviewForm()
	{
		...
		{				
			...
			ilUtil::moveUploadedFile($incoming["tmp_name"],
				$org_name,
				$path.$org_name,
				false);
            ...
            if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                ...
                copyrightHelper::_updateCopyRightOptionByObjectId(
                    $this->ass->getExerciseId(),
                    $this->form->getItemByPostVar("copy_right_option_".$this->getId())->getValue(),
                    $this->ass->getId(),
                    "peer_feedback|".basename($path.$org_name)."|".$this->getId()."|".$this->peer_id);
            }
