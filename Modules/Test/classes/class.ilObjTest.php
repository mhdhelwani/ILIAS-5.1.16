...
*/
	...
	public function removeTestResultsByActiveIds($activeIds)
	{
        ...
        // get solution ids by active ids
        $query = "SELECT * FROM tst_solutions WHERE $IN_activeIds";
        $result = $ilDB->query($query);
        $solutions = array();

        while ($res  = $ilDB->fetchAssoc($result)){
            $solutions[$res["active_fi"]][] = [
                "solution_id" => $res["solution_id"],
                "value1" => $res["value1"],
                "value2" => $res["value2"],
                "question_id" => $res["question_di"]
            ];
        }
		...
		foreach ($activeIds as $active_id)
		{
			...
			foreach ($solutions[$active_id] as $solution){
                copyrightHelper::_deleteCopyRightOptionByObjectId(
                    $this->getId(),
                    $active_id,
                    "tst_solutions|" . $solution["value1"] . "|" .
                    $solution["solution_id"] . "|" . $this->getTestId() .
                    "|" . $solution["value2"] . "|" . $solution["question_id"]);
            }
