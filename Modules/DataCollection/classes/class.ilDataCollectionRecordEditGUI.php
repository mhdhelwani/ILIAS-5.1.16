	...
	public function initForm() {
		...
		foreach ($allFields as $field) {
            ...
            if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                ...
                if (in_array($field->getDatatypeId(), [ilDataCollectionDatatype::INPUTFORMAT_FILE, ilDataCollectionDatatype::INPUTFORMAT_MOB])) {
                    $copyRightRadio = copyrightHelper::_getCopyRightActiveOptionRadio("select_list_title", "copy_right_option_" . $field->getId());
                    if (!$field->getRequired()) {
                        $copyRightRadio->setRequired(false);
                    }
                    $item->addSubItem($copyRightRadio);
                }
            }
			...
			foreach ($allFields as $field) {
                ...
                if (in_array($field->getDatatypeId(), [ilDataCollectionDatatype::INPUTFORMAT_FILE, ilDataCollectionDatatype::INPUTFORMAT_MOB])) {
                    if ($field->getDatatypeId() == ilDataCollectionDatatype::INPUTFORMAT_FILE) {
                        ...
                        $mob = new ilObjMediaObject($record_obj->getRecordFieldValue($field->getId()));
                        $mediaItem = $mob->getMediaItem("Standard");
                        $subId = $mediaItem->getId();
                        $fileLocation = "mob_Standard";
                    }

                    $copyRightValue = copyrightHelper::_getCopyRightValue(
                        $record_obj->getRecordFieldValue($field->getId()),
                        $subId,
                        $fileLocation
                    );
                    $this->form->getItemByPostVar('copy_right_option_' . $field->getId())->setValue($copyRightValue);
                }
