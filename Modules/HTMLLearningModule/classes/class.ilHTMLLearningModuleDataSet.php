		...
		switch ($a_entity)
		{
			case "htlm":
                ...
                if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                    $dir_files = iterator_to_array(new RecursiveIteratorIterator(new RecursiveDirectoryIterator($target_dir)));
                    ...
                    foreach ($dir_files as $f => $d) {
                        ...
                        if (!is_dir($f)) {
                            $sub_dir = str_replace(
                                "\\" ,
                                "/",
                                substr($pi["dirname"], strlen($target_dir))
                            );
                            ...
                            copyrightHelper::_updateCopyRightOptionByObjectId(
                                ...
                                0,
                                "lms_html_file|" .
                                $pi["basename"] . "|" . ((!empty($cur_subdir)) ? "/" . $cur_subdir . $sub_dir : $sub_dir));
                        }
                    }
                }
