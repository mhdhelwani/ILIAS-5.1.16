	...
	function initAddCastItemForm($a_mode = "create")
	{
		...
		foreach (ilObjMediaCast::$purposes as $purpose)
		{
            ...
            if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                ...
                $copyRightOptionRadio = copyrightHelper::_getCopyRightActiveOptionRadio("select_list_title", "copy_right_option_" . $purpose);
                $copyRightOptionRadio->setRequired(false);
                $file->addSubItem($copyRightOptionRadio);
            }
    		...
    		if ($purpose != "Standard")
    		{
                ...
                if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                    ...
                    $copyRightOptionRadio = copyrightHelper::_getCopyRightActiveOptionRadio("select_list_title", "copy_right_option_pp");
                    $copyRightOptionRadio->setRequired(false);
                    $pp->addSubItem($copyRightOptionRadio);
                }
	...
	public function getCastItemValues()
	{
		...
		if ($ppic != "")
		{
            ...
            if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                ...
                $this->form_gui->getItemByPostVar("copy_right_option_pp")->setValue(copyrightHelper::_getCopyRightValue(
                    $mob->getId(),
                    0,
                    "mob_preview_pic"));
            }
		...
		if (count ($mediaItems) > 0)
		{
		    foreach ($mediaItems as $med) 
		    {
                ...
                if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                    ...
                    $this->form_gui->getItemByPostVar("copy_right_option_".$med->getPurpose())->setValue(copyrightHelper::_getCopyRightValue(
                        $mob->getId(),
                        $med->getId(),
                        "mob_".$med->getPurpose()));
                }
		...
		else
		{
                ...
                if ($mediaItem->getLocationType() === "LocalFile") {
                    if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                        copyrightHelper::_updateCopyRightOptionByObjectId(
                            $mob->getId(),
                            $this->form_gui->getInput("copy_right_option_" . $mediaItem->getPurpose()),
                            $mediaItem->getId(),
                            "mob_" . $mediaItem->getPurpose()
                        );
                    }
            ...
            if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                if ($prevpic["size"] > 0) {
                    copyrightHelper::_updateCopyRightOptionByObjectId(
                        $mob->getId(),
                        $this->form_gui->getInput("copy_right_option_pp"),
                        0,
                        "mob_preview_pic"
                    );
                }
