	...
	*/
	...
	function getPreview($show_question_only = FALSE, $showInlineFeedback = false)
	{
        ...
        if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
            ...
            $template->setCurrentBlock("copy_right_option");
            ...
            $template->setVariable("TEXT_COPY_RIGHT", $copyRightPlugin->txt('select_list_title'));
            $template->setVariable("COPY_RIGHT_PLUGIN_IS_ACTIVE", ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')));
            $template->setVariable("COPY_RIGHT_OPTIONS", $copyRightRadio);
            $template->parseCurrentBlock();
        }
