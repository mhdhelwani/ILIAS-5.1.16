		...
		if (count($solution_array))
		{
			...
			if (strcmp($solution_array["type"], "file") == 0)
			{
                ...
                if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
                    ...
                    $copy_right_radio = copyrightHelper::_getCopyRightActiveOptionRadio("select_list_title", "copy_right_option");
                    $file->addSubItem($copy_right_radio);

                    if (strlen($solution_array["value"]["name"]) !== 0 || ($_FILES["file"]["tmp_name"] && $file->checkInput())) {
                    ...
                    if (strlen($solution_array["value"]["name"]) !== 0) {
                        $copy_right_radio->setValue(
