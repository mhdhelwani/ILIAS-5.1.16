			...
			foreach ($_SESSION["qpl_clipboard"] as $question_object)
			{
				if (strcmp($question_object["action"], "move") == 0)
				{
					...
					if ($result->numRows() == 1)
					{
                        ...
                        $questionType = assQuestion::_instantiateQuestion($question_object["question_id"])->getQuestionType();

                        if ($questionType === "assJavaApplet"){
                        	$questionType = "java_applet";
						} else if ($questionType === "assImagemapQuestion") {
                        	$questionType = "image_map";
						} else if ($questionType === "assFlashQuestion") {
                        	$questionType = "flash";
						}
