...
class ilObjQuestionPoolGUI extends ilObjectGUI
{
	...
	*/
	function uploadQplObject($questions_only = false)
	{
		...
		$this->ctrl->setParameter($this, 'copy_right_option', $_REQUEST['copy_right_option']);
	...
	*/
	function importVerifiedFileObject()
	{
		...
		if (is_file($_SESSION["qpl_import_dir"].'/'.$_SESSION["qpl_import_subdir"]."/manifest.xml"))
		{
			...
			$imp->importObject($newObj, $fullPath, $fileName, "qpl", "Modules/TestQuestionPool", true /* patch start 'mhd_helwani' */, $_REQUEST["copy_right_option"] /* patch end 'mhd_helwani' */);
		}
		else
		{
			...
			$qtiParser->setCopyRightOption($_GET["copy_right_option"]);
			...
			if (strlen($_SESSION["qpl_import_xml_file"]))
			{
                ...
                $contParser->setCopyRightOption($_GET["copy_right_option"]);
	...
	*/
	...
	protected function getImportQuestionsForm()
	{
        ...
        if ($GLOBALS['ilPluginAdmin']->isActive(IL_COMP_SERVICE, 'UIComponent', 'uihk', 'CopyRight')) {
            ...
            $copyRightRadio = copyrightHelper::_getCopyRightActiveOptionRadio("select_list_title", "copy_right_option");
            $form->addItem($copyRightRadio);
        }
